                ...
                return (
                  <div
                    ...
                    className={`group relative flex flex-col border transition-all duration-300 ${compact ? 'rounded-[16px]' : 'rounded-[22px]'} ${isBest
                      ...
                      : isPopular
                        ? 'border-[#f3f0ed]/[0.1] bg-[#16161a] hover:border-[#f3f0ed]/[0.15]'
                  ...
                  >
                    ...
                    {isBest && (
                      <>
                        <div
                          ...
                          style={{
                            background: 'linear-gradient(180deg, rgba(225,29,42,0.15) 0%, rgba(225,29,42,0) 50%)',
                          }}
                        />
                        <div
                          className={`pointer-events-none absolute -top-[1px] h-[2px] ${compact ? 'left-6 right-6' : 'left-8 right-8'}`}
                          style={{
                            background: 'linear-gradient(90deg, transparent, rgba(225,29,42,0.6), transparent)',
                          }}
                        />
                      </>
                    )}
                    ...
                    {isPopular && (
                      <div className={`absolute left-1/2 z-10 -translate-x-1/2 ${compact ? '-top-2.5' : '-top-3'}`}>
                        <div className={`flex items-center gap-1 rounded-full border border-[#f3f0ed]/15 bg-[#111113] shadow-lg ${compact ? 'px-3 py-0.5' : 'px-4 py-1'}`}>
                    ...
                    {isBest && (
                      <div className={`absolute left-1/2 z-10 -translate-x-1/2 ${compact ? '-top-2.5' : '-top-3.5'}`}>
                        <div className={`flex items-center gap-1 rounded-full bg-[#e11d2a] shadow-[0_0_30px_rgba(225,29,42,0.4)] ${compact ? 'px-3 py-0.5' : 'px-5 py-1.5'}`}>
                          <Crown className={`${compact ? 'h-2.5 w-2.5' : 'h-3.5 w-3.5'} text-[#0a0a0b]`} />
                          <span className={`font-extrabold uppercase tracking-[0.08em] text-[#0a0a0b] ${compact ? 'text-[10px]' : 'text-[11px]'}`}>
                            {t('badges.bestValue')}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className={`relative flex flex-1 flex-col ${compact ? 'p-3' : 'p-5 sm:p-6'}`}>
                      ...
                      <div className="flex items-center gap-2">
                        <div className={`flex items-center justify-center rounded-lg ${compact ? 'h-6 w-6' : 'h-8 w-8'} ${isBest ? 'bg-[#e11d2a]/15' : 'bg-[#f3f0ed]/[0.05]'
                          }`}>
                          <PkgIcon className={`${compact ? 'h-3 w-3' : 'h-4 w-4'} ${isBest ? 'text-[#e11d2a]' : 'text-[#f3f0ed]/40'}`} />
                        </div>
                        <div>
                      ...
                      <div className={compact ? 'mt-3' : 'mt-5 min-h-[55px]'}>
                        {discountPct > 0 && (
                          <div className={`flex items-center gap-1.5 ${compact ? 'mb-0.5' : 'mb-1'}`}>
                            ...
                            <span className={`rounded-md bg-[#e11d2a]/15 font-bold text-[#e11d2a] ${compact ? 'px-1 py-0.5 text-[10px]' : 'px-1.5 py-0.5 text-[9px]'}`}>
                              {t('badges.discountOff', { pct: discountPct })}
                            </span>
                          </div>
                        )}
                        <div className="flex items-baseline gap-0.5">
                          <span className={`font-extrabold leading-none tracking-tight ${compact ? 'text-[22px]' : 'text-[22px]'} ${isBest ? 'text-[#e11d2a]' : 'text-[#f3f0ed]'
                            }`}>
                      ...
                      <div className={`flex items-center gap-1.5 rounded-lg border ${compact ? 'mt-2 px-2.5 py-1.5' : 'mt-4 rounded-xl px-3.5 py-2.5'} ${isBest ? 'bg-[#e11d2a]/10 border-[#e11d2a]/15' : 'bg-[#f3f0ed]/[0.03] border-[#f3f0ed]/[0.05]'
                        }`}>
                        <Coins className={`${compact ? 'h-3 w-3' : 'h-4 w-4'} ${isBest ? 'text-[#e11d2a]' : 'text-[#f3f0ed]/40'}`} />
                        <div>
                          <span className={`font-extrabold tabular-nums ${compact ? 'text-[15px]' : 'text-[16px]'} ${isBest ? 'text-[#e11d2a]' : 'text-[#f3f0ed]'
                            }`}>
                      ...
                      {/* Savings callout */}
                      {savingsPct > 0 && (
                        <p className={`font-medium text-[#e11d2a]/70 ${compact ? 'mt-1.5 text-[12px]' : 'mt-3 text-[11px]'}`}>
