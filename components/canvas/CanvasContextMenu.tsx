...
export function CanvasContextMenu({ children, onAddPanel }: CanvasContextMenuProps) {
    ...
    return (
        <ContextMenu>
            ...
            <ContextMenuContent
                className="w-64 overflow-hidden rounded-2xl border border-[#f3f0ed]/[0.08] bg-[#111113] p-1.5 shadow-2xl shadow-black/60 backdrop-blur-xl"
            >
                ...
                {PANEL_GROUPS.map((group) => {
                        ...
                        return (
                            <ContextMenuItem
                                ...
                                className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 outline-none transition-all ${
                                    isComingSoon
                                        ? 'opacity-50'
                                        : 'cursor-pointer focus:bg-[#3a0f16]/40 data-[highlighted]:bg-[#3a0f16]/40'
                                }`}
                            >
                                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-[#f3f0ed]/[0.08] bg-[#3a0f16]/30 transition-all group-focus:border-[#e11d2a]/30 group-focus:bg-[#e11d2a]/10 group-data-[highlighted]:border-[#e11d2a]/30 group-data-[highlighted]:bg-[#e11d2a]/10">
                                    <PanelIcon className="h-4 w-4 text-[#f3f0ed]/50 transition-colors group-focus:text-[#e11d2a] group-data-[highlighted]:text-[#e11d2a]" />
                                </div>
                    ...
                    return (
                        <ContextMenuSub key={group.id}>
                            <ContextMenuSubTrigger
                                className="group flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 outline-none transition-all focus:bg-[#3a0f16]/40 data-[state=open]:bg-[#3a0f16]/40"
                            >
                                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-[#e11d2a]/20 bg-[#e11d2a]/[0.06] transition-all">
                                    <GroupIcon className="h-4 w-4 text-[#e11d2a]" />
                                </div>
                            ...
                            >
                                {group.panels.map((panel) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-[#f3f0ed]/[0.08] bg-[#3a0f16]/30 transition-all group-focus:border-[#e11d2a]/30 group-focus:bg-[#e11d2a]/10 group-data-[highlighted]:border-[#e11d2a]/30 group-data-[highlighted]:bg-[#e11d2a]/10">
                                                <Icon className="h-4 w-4 text-[#f3f0ed]/50 transition-colors group-focus:text-[#e11d2a] group-data-[highlighted]:text-[#e11d2a]" />
                                            </div>
                                            <div className="flex flex-col flex-1 min-w-0">
                                                <span className="flex items-center gap-2 text-sm font-semibold text-[#f3f0ed]/90">
                                                    {t(`items.${panel.contextKey}.label`)}
