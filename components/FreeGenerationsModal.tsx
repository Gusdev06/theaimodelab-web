...
// Nomes dos modelos são próprios (marca) — iguais em todos os idiomas.
export const FREE_GEN_LABELS: [FreeGenerationType, string][] = [
  ['NB2', 'Nano Banana 2'],
...
export function FreeGenerationsModal({ open, onClose, items, total }: FreeGenerationsModalProps) {
  const router = useRouter();
  const t = useTranslations('freeGens.modal');
      ...
      >
        <div className="relative overflow-hidden rounded-2xl border border-[#e11d2a]/20 bg-gradient-to-br from-[#1f2a1c] via-[#111113] to-[#111113] p-8">
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <div className="mb-6 w-full rounded-xl border border-[#e11d2a]/20 bg-[#e11d2a]/5 p-4 text-left">
              <div className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-[#f3f0ed]/50">
                {t('listTitle')}
              </div>
              <div className="flex max-h-52 flex-col gap-1.5 overflow-y-auto pr-1 scrollbar-app">
                {items.map((item) => (
                  <div
                    key={item.label}
                    className="flex items-center justify-between gap-3 rounded-lg bg-[#111113]/40 px-3 py-2"
                  >
                    <span className="truncate text-sm text-[#f3f0ed]/85">{item.label}</span>
                    <span className="shrink-0 rounded-md bg-[#e11d2a]/15 px-2 py-0.5 text-xs font-semibold text-[#e11d2a]">
                      ×{item.count}
                    </span>
                  </div>
                ))}
